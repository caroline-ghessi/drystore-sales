...
// Cálculo de preparação de piso MAPEI - Autonivelantes e Regularização
export function calculateFloorPreparationMapei(input: FloorPreparationMapeiInput): FloorPreparationMapeiResult {
  const {
    area,
    currentCondition,
  ...
  // 2. OBTER ESPECIFICAÇÕES DO PRODUTO
  const productSpecs = getProductSpecifications(productType);
  ...
  // 4. CÁLCULO PRINCIPAL - Quantidade de produto
  // Fórmula: Volume (m³) × Densidade (kg/m³) × Fatores de Correção
  ...
  // 5. CALCULAR PRIMER (se necessário)
  const primer = calculatePrimerFloor(area, primerRequired, primerType, primerDilution, baseSubstrate);

  // 6. GERAR LISTA QUANTIFICADA
  ...
  const tools = getRequiredTools(area, productType, preparationType);
  tools.forEach(tool => {
    quantified_items.push({
      name: tool.name,
...
// Especificações dos produtos MAPEI
function getProductSpecifications(productType: string) {
  const products = {
    'ultraplan_eco': {
      name: 'ULTRAPLAN ECO',
  ...
  return products[productType as keyof typeof products] || products.ultraplan_eco;
}
...
// Calcular primer para piso
function calculatePrimerFloor(
  area: number,
  required: boolean,
  type?: string,
  dilution?: string,
  ...
  const selectedPrimer = primers[type as keyof typeof primers] || primers.primer_g;
...
function getRequiredTools(area: number, productType: string, preparationType: string) {
  const tools = [];

  // Furadeira com batedor (obrigatório)
  tools.push({
    name: 'Furadeira com batedor para argamassa',
    description: 'Mínimo 1200W para mistura adequada',
    quantity: 1,
    unit: 'unidade'
  });

  // Baldes (baseado na área)
  const bucketsNeeded = Math.ceil(area / 50); // 1 balde a cada 50m²
  tools.push({
    name: 'Balde plástico graduado 20L',
    description: 'Para dosagem correta da água',
    quantity: bucketsNeeded,
    unit: 'unidade'
  });

  // Rolo desareador
  tools.push({
    name: 'Rolo desareador com cabo',
    description: 'Para eliminação de bolhas de ar',
    quantity: 1,
    unit: 'unidade'
  });

  // Sapato de prego (para grandes áreas)
  if (area > 20) {
    tools.push({
      name: 'Sapato de prego',
      description: 'Para caminhar sobre o produto fresco',
      quantity: 1,
      unit: 'par'
    });
  }
