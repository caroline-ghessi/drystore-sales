...
export function mapToOpportunityFields(
  ...
  if (coaching) {
    // Helper para acessar dados aninhados (reusa a função existente)
    const get = (obj: unknown, ...paths: string[]): unknown => {
      for (const path of paths) {
        const keys = path.split('.');
        let value: unknown = obj;
    ...
    if (Array.isArray(riskAlerts) && riskAlerts.length > 0) {
      ...
      const hasHighRisk = riskAlerts.some((r: unknown) => 
        (r as Record<string, unknown>).probability === 'high'
      );
      const hasMediumRisk = riskAlerts.some((r: unknown) => 
        (r as Record<string, unknown>).probability === 'medium'
      );
      if (hasHighRisk) {
