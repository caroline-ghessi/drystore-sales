...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'forro_knauf' AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'product_category')) THEN
        ALTER TYPE product_category ADD VALUE 'forro_knauf';
    END IF;
END $$;
